...
const FlashcardManager = ({ documentId }) => {
  ...
  // ✅ Accept cardIndex as a parameter
    ...
    if (flashcardSets.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center py-16 px-6">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-linear-to-br from-indigo-500/20 to-indigo-600/10 border border-indigo-500/20 mb-4">
          ...
          <button
            ...
            disabled={generating}
            className="group inline-flex items-center gap-2 px-6 h-12 bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 disabled:opacity-70 disabled:cursor-not-allowed text-white font-medium text-sm rounded-xl shadow-lg shadow-indigo-500/25 transition-all duration-200"
          >
            {generating ? (
              <>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={handleGenerateFlashcards}
            disabled={generating}
            className="inline-flex items-center gap-2 px-5 h-11 bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 disabled:opacity-70 disabled:cursor-not-allowed text-white font-medium text-sm rounded-xl shadow-lg shadow-indigo-500/25 transition-all duration-200"
          >
            {generating ? (
              <>
                <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {flashcardSets.map((set) => (
            <div
              ...
              className="relative group bg-white border-2 border-slate-100 hover:border-indigo-400 rounded-2xl p-5 cursor-pointer transition-all duration-200 hover:shadow-md hover:shadow-indigo-500/10"
            >
              {/* Delete Button */}
              <button
                onClick={(e) => handleDeleteRequest(e, set)}
                className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all duration-150"
              >
              ...
              <div className="flex flex-col gap-3">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-linear-to-br from-indigo-500/20 to-indigo-600/10 border border-indigo-500/20">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button
              ...
              disabled={deleting}
              className="px-5 h-11 bg-linear-to-r from-rose-500 to-red-500 hover:from-rose-600 hover:to-red-600 disabled:opacity-70 disabled:cursor-not-allowed text-white font-medium text-sm rounded-xl shadow-lg shadow-red-500/25 transition-all duration-150"
            >
              {deleting ? (
